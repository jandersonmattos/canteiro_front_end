...
type ApiBudgetScheduleResponse = {
  id: string;
  budget_id: string;
  name: string;
...
export default function EditBudgetPage() {
    ...
    async function loadBudget() {
      ...
      try {
        ...
        const scheduleResponse = await fetch(
          `${process.env.NEXT_PUBLIC_CANTEIRO_API_URL}/budgets/${budgetId}/schedule`
        );

        if (scheduleResponse.ok) {
          const scheduleData: ApiBudgetScheduleResponse = await scheduleResponse.json();

          const serviceIdByBudgetItemId: Record<string, string> = {};
          budgetData.items.forEach((item) => {
            if (item.id && item.service_id) {
              serviceIdByBudgetItemId[item.id] = item.service_id;
            }
          });

          const mappedScheduleTasks: Record<string, ScheduleTaskConfig> = {};
          const today = formatDateYYYYMMDD(new Date());

          scheduleData.tasks.forEach((task) => {
            const serviceIdFromItem = task.budget_item_id
              ? serviceIdByBudgetItemId[task.budget_item_id]
              : undefined;

            const serviceIdFromCode = task.code
              ? itemsByCode[task.code]?.service_id || undefined
              : undefined;

            const serviceId = serviceIdFromItem || serviceIdFromCode;

            if (!serviceId || !reconstructedServices[serviceId]) {
              return;
            }

            const startDate = normalizeDateString(task.start_date) || today;
            const duration = Math.max(
              1,
              Number(task.duration_days || 0) ||
                calculateDurationDays(startDate, task.end_date)
            );

            mappedScheduleTasks[serviceId] = {
              startDate,
              duration,
            };
          });

          setScheduleTasks(mappedScheduleTasks);
          setScheduleExists(true);
        } else if (scheduleResponse.status === 404) {
          setScheduleTasks({});
          setScheduleExists(false);
        } else {
          console.error("Erro ao carregar cronograma:", scheduleResponse.status);
        }
  ...
  useEffect(() => {
    async function loadAbcData() {
      ...
      try {
        setAbcLoading(true);
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_CANTEIRO_API_URL}/budgets/${budgetId}/abc`
        );

        if (!response.ok) {
          throw new Error("Erro ao carregar Curva ABC");
        }
        ...
        toast.error(
          error instanceof Error ? error.message : "Erro ao carregar Curva ABC"
        );
      } finally {
        setAbcLoading(false);
      }
  ...
  const buildSchedulePayload = useCallback(() => {
    const today = formatDateYYYYMMDD(new Date());
    const groupedServices = buildCategoryHierarchy(selectedServicesList);
    const orderedServices = groupedServices.flatMap((group) => group.items);

    const tasks = orderedServices.map(({ service }, index) => {
      const taskConfig = scheduleTasks[service.id];
      const startDate = taskConfig?.startDate || today;
      const duration = Math.max(1, Number(taskConfig?.duration || 1));
      ...
      return {
        ...
        name: service.description || "Tarefa sem nome",
        description: null,
        task_type: "TASK",
        ...
        planned_cost: Number(service.total_cost || 0),
        actual_cost: 0,
        responsible: null,
        color: null,
        notes: null,
    ...
    const dependencies = groupedServices.flatMap((group) => {
      const serviceIds = group.items.map(({ service }) => service.id);
      ...
      return serviceIds
        .map((serviceId, index) => {
          ...
          return {
            predecessor_temp_id: serviceId,
            successor_temp_id: nextId,
            dependency_type: "FS",
            lag_days: 0,
          };
        })
    ...
    const startDates = tasks.map((task) => task.start_date).filter(Boolean);
    const endDates = tasks.map((task) => task.end_date).filter(Boolean);

    const scheduleStart = startDates.length > 0 ? startDates.sort()[0] : today;
    const scheduleEnd = endDates.length > 0 ? endDates.sort()[endDates.length - 1] : today;

    return {
      name: `Cronograma ${form.numero || budgetId}`,
      description: form.observacoes?.trim() || null,
      start_date: scheduleStart,
      end_date: scheduleEnd,
      baseline_start_date: scheduleStart,
      baseline_end_date: scheduleEnd,
      status: "PLANNING",
      tasks,
      dependencies,
    };
  }, [selectedServicesList, scheduleTasks, form.numero, form.observacoes, budgetId]);

  const saveBudgetSchedule = useCallback(async () => {
    if (!budgetId || selectedServicesList.length === 0) {
      return;
    }
    ...
    if (!response.ok) {
      ...
      const detailMessage =
        responseData && typeof responseData.detail === "string"
          ? responseData.detail
          : "Nao foi possivel salvar o cronograma";

      throw new Error(detailMessage);
    }

    setScheduleExists(true);
  }, [budgetId, selectedServicesList.length, buildSchedulePayload, scheduleExists]);

  async function handleSaveScheduleOnly() {
    if (selectedServicesList.length === 0) {
      toast.error("Selecione pelo menos 1 serviço para montar o cronograma");
      return;
    }

    try {
      setSavingSchedule(true);
      await saveBudgetSchedule();
  ...
  async function handleUpdateBudget() {
    ...
    try {
      ...
      toast.success("Orçamento e cronograma atualizados com sucesso");
    } catch (error) {
      ...
      "
            ...
            {activeTab === "cronograma" && (
              <ScheduleView
                ...
                onSaveSchedule={handleSaveScheduleOnly}
              ...
              <div>
                {abcLoading ? (
                  <div className="rounded-[32px] border border-white/10 bg-white/[0.03] p-16 text-center">
                    <div className="w-12 h-12 rounded-full border-4 border-white/10 border-t-emerald-500 animate-spin mx-auto mb-6"></div>
                    <p className="text-zinc-400">Carregando Curva ABC...</p>
                  </div>
                ) : abcData ? (
                  <>
                    {/* RESUMO */}
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                      <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
                        <p className="text-sm text-zinc-400 mb-2">Total geral</p>
                        <h3 className="text-2xl font-bold">
                          R$ {abcData.total_cost?.toLocaleString("pt-BR", {
                      ...
                      <div className="rounded-[28px] border border-amber-500/20 bg-amber-500/10 p-6">
                        <p className="text-sm text-amber-300 mb-2">Itens Classe A</p>
                        <h3 className="text-2xl font-bold text-amber-300">
                          {abcData.summary?.class_a_items || 0}
                        </h3>
                      </div>

                      <div className="rounded-[28px] border border-blue-500/20 bg-blue-500/10 p-6">
                        <p className="text-sm text-blue-300 mb-2">Itens Classe B</p>
                        <h3 className="text-2xl font-bold text-blue-300">
                          {abcData.summary?.class_b_items || 0}
                        </h3>
                      </div>

                      <div className="rounded-[28px] border border-green-500/20 bg-green-500/10 p-6">
                        <p className="text-sm text-green-300 mb-2">Itens Classe C</p>
                        <h3 className="text-2xl font-bold text-green-300">
                          {abcData.summary?.class_c_items || 0}
                        </h3>
                      </div>
                    </div>
                    ...
                    <div className="rounded-[28px] border border-white/10 bg-white/[0.03] overflow-hidden">
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          ...
                          <tbody>
                            {abcData.items?.map((item: any, index: number) => {
                              let classColor = "text-zinc-400";
                              let classBg = "bg-zinc-500/10";
                              let classLabel = "—";

                              if (item.classification === "A") {
                                classColor = "text-amber-300";
                                classBg = "bg-amber-500/10";
                                classLabel = "A";
                              } else if (item.classification === "B") {
                                classColor = "text-blue-300";
                                classBg = "bg-blue-500/10";
                                classLabel = "B";
                              } else if (item.classification === "C") {
                                classColor = "text-green-300";
                                classBg = "bg-green-500/10";
                                classLabel = "C";
                              }
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4 text-sm text-zinc-300 max-w-xs">
                                  ...
                                  <td className="px-6 py-4 text-sm text-right text-zinc-300 font-medium">
                                  ...
                                  <td className="px-6 py-4 text-center">
                                    <div
                                      className={`inline-flex items-center justify-center w-8 h-8 rounded-lg ${classBg} font-semibold ${classColor}`}
                                    >
                                      {classLabel}
                                    </div>
                                  </td>
                  ...
                  <div className="rounded-[32px] border border-white/10 bg-white/[0.03] p-16 text-center">
                    <div className="w-20 h-20 rounded-[28px] bg-white/[0.03] border border-white/10 flex items-center justify-center mx-auto mb-8">
                      <Calculator size={38} className="text-zinc-600" />
                    </div>
                    <h3 className="text-2xl font-bold mb-3">Curva ABC não disponível</h3>
                    <p className="text-zinc-500 max-w-2xl mx-auto leading-relaxed">
                      Não foi possível carregar os dados da Curva ABC. Tente novamente.
                    </p>
                  </div>
                )}
              </div>
            ...
            {activeTab !== "dados" && activeTab !== "cronograma" && activeTab !== "curva" && (
              <div className="rounded-[32px] border border-white/10 bg-white/[0.03] p-16 text-center">
                <div className="w-20 h-20 rounded-[28px] bg-white/[0.03] border border-white/10 flex items-center justify-center mx-auto mb-8">
...
function normalizeDateString(dateValue?: string | null) {
  if (!dateValue) {
    return "";
  }
...
function calculateDurationDays(startDate: string, endDate?: string | null) {
  if (!endDate) {
    return 1;
  }

  const start = new Date(startDate);
  const end = new Date(normalizeDateString(endDate) || startDate);
  const diffInMs = end.getTime() - start.getTime();
  const diffInDays = Math.ceil(diffInMs / (1000 * 60 * 60 * 24));

  return Math.max(1, Number.isFinite(diffInDays) ? diffInDays : 1);
}
